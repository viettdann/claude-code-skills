...
This is a collection of **Agent Skills** - specialized AI agents for security auditing, code quality analysis, and documentation access. Each skill is a self-contained module with its own SKILL.md definition, documentation, and standalone scripts.

**What are Agent Skills?** Skills package expertise into discoverable capabilities. They are **model-invoked** (agents autonomously decide when to use them) rather than user-invoked like slash commands.

## Official Documentation

This repository follows the official Agent Skills specification:
...
The `knowledge/` directory contains official documentation for reference:

- `claude_code_docs_skills.md` - Complete official skills documentation
- `claude_code_docs_map.md` - Hierarchical map of all docs with links
...
**Manual testing:**
1. Ask the agent a question that should trigger the skill
...
- `description` (required): Brief description of what the skill does and **when to use it**. Max 1024 chars. **Critical for discovery**.
- `allowed-tools` (optional): Restricts which tools the agent can use when this skill is active.
...
- The agent will read referenced files automatically when following those links

This approach:
- Reduces initial token usage
- Allows comprehensive documentation without overwhelming context
- Enables deep-dive when the agent needs specific details
...
1. **AI-Guided Analysis** (Primary)
   - Agent reads SKILL.md and executes analysis using allowed tools
...
5. **Test activation**
   - Ask the agent questions matching your description
...
2. **Changes take effect on restart**
   - If the agent is running, restart it to load updates
...
5. **Verify skill is discovered**
   Ask the agent: "What skills are available?" or "List all available skills"

### Multiple Skills Conflicting

**Problem**: Agent uses the wrong skill or seems confused between similar skills.
...
1. **Check dependencies availability**
   - Agent will automatically install required dependencies or ask for permission
...
- **Secrets Detection**: Use the scanning-for-secrets skill to audit for hardcoded credentials
- **Framework Security**: Use framework-specific skills (ABP, Next.js) for security best practices
- **Validation**: Always validate findings in context to avoid false positives
- **Dependencies**: Document all external dependencies in skill README.md files
- **Permissions**: Use `allowed-tools` in SKILL.md to restrict tool usage for security-sensitive skills
...
- Cursor
- Aider
- And other agents following the agents.md specification

For agent-specific configuration, see:
- **Claude Code**: Use `.claude/skills/` directory structure
- **Aider**: Automatically loads this file - see [conventions](https://aider.chat/docs/usage/conventions.html#always-load-conventions)
- **Gemini CLI**: See [configuration settings](https://github.com/google-gemini/gemini-cli/blob/main/docs/get-started/configuration.md#available-settings-in-settingsjson)
...
**Repository format**: Agent Skills v1.0
**Skill format version**: 2025-01 (following official specification)
**Compatible with**: Claude Code 1.0+, and other agents following agents.md specification
